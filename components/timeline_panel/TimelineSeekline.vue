...
export default class TimelineSeekline extends Vue {
  ...
  get array(): number[] {
    ...
    for (let x = 0; x < this.showLength; x += TIME_TEXT_MAX_WIDTH / this.scale) {
      a.push(ss + x)
    }
    return a
  }
  ...
  // 把游标移动的位置信息（所在时间currentTime）发送到父组件，到editor组件后共享currentTime数据
  // 该数据在该组件部分为自定义属性pos，再向下传递，完成游标样式的更改
  // 核心理念为：js代码动态更改css样式
  movePos(e: MouseEvent) {
    this.$emit('changePos', roundToFrame(this.start + e.offsetX / this.scale, this.fps) + 0.0001)
    this.isDrag = true
  }
