<template>
  <div class="timeline-strip" :class="rootClass" :style="lineStyle" @click="click" @pointerdown="dragStart">
    <div :style="over" />
    <div class="strip">
...
export default class TimelineStrip extends Vue {
  ...
  // 该返回值为具体strip条的位置样式，动态渲染到strip轴上
  ...
  get over(): Partial<CSSStyleDeclaration> {
    return {
      ...
      background: this.selected ? 'rgba(255 255 255 / .2)' : this.valid ? '' : 'rgba(255 0 0 / .2)'
    }
  }
  ...
  beforeDestroy() {
    for (let i = 0; i < this.layers.length; i++) {
      this.layers[i].removeEventListener('mouseenter', this.mouseEnterEvents[i])
    }
  }
  ...
  // strip条拖动实现函数，追踪鼠标事件MouseEvent完成
