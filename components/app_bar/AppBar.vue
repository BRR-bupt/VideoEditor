<template>
  <div class="app-bar">
    ...
    <input ref="input" type="file" style="display: none" @change="changeOpenProject" />

    <div style="margin: auto 4px auto auto">
      <sp-action-button :quiet="true" @click="renderVideo">
        <sp-icon name="BoxExport" style="width: 12px" />
      </sp-action-button>
      <sp-action-button :quiet="true" data-vega-settings-button @click="isOpenProjectSettings = true">
    ...
    <project-setting-modal :isOpenSync.sync="isOpenProjectSettings" :projectSync.sync="project" />
  </div>
</template>
